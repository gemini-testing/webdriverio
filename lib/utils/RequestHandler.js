...
import getCommandDataFromRequest from '../helpers/getCommandDataFromRequest'
...
function getStartPath(options) {
    return options.path === '/' ? '' : options.path || '/wd/hub'
}
...
class RequestHandler {
    constructor (options, eventHandler, logger) {
        this.sessionID = null
        this.startPath = getStartPath(options)
    ...
    createOptions (requestOptions, data) {
        ...
        newOptions.uri = url.parse(
            ...
            (requestOptions.gridCommand ? this.gridApiStartPath : getStartPath(this.defaultOptions)) +
        ...
        return new Promise((resolve, reject) => {
            request(fullRequestOptions, (err, response, body) => {
                ...
                if (retryCount >= totalRetryCount) {
                    ...
                    if (err) {
                        const { cmdName, cmdData } = getCommandDataFromRequest(fullRequestOptions)
                        const withArgs = cmdData ? ` with args "${cmdData}"` : ''

                        const orgStatusMessage = (err.code === 'ESOCKETTIMEDOUT')
                            ? `${err.message}: when running "${cmdName}"${withArgs}`
                            : `${err.message}`
