...
var _getCommandDataFromRequest = require('../helpers/getCommandDataFromRequest');

var _getCommandDataFromRequest2 = _interopRequireDefault(_getCommandDataFromRequest);
            ...
            return new _promise2.default(function (resolve, reject) {
                (0, _request3.default)(fullRequestOptions, function (err, response, body) {
                    ...
                    if (retryCount >= totalRetryCount) {
                        ...
                        if (err) {
                            var _getCommandDataFromRe = (0, _getCommandDataFromRequest2.default)(fullRequestOptions),
                                cmdName = _getCommandDataFromRe.cmdName,
                                cmdData = _getCommandDataFromRe.cmdData;

                            var withArgs = cmdData ? ` with args "${cmdData}"` : '';

                            var orgStatusMessage = err.code === 'ESOCKETTIMEDOUT' ? `${err.message}: when running "${cmdName}"${withArgs}` : `${err.message}`;
