...
var WebdriverIO = function WebdriverIO(args, modifier) {
    ...
    function fail(e, onRejected) {
        ...
        var customStack = commandList.map(function (command) {
            return `    at ${command.stack}`;
        }).reverse().slice(0, stackDepth).join('\n');
    ...
    unit.lift = function (name, func) {
        ...
        commandGroup[name] = function () {
            ...
            client.stack = `${name}(${_sanitize2.default.args(args)}) (${fileAndPosition})`;
